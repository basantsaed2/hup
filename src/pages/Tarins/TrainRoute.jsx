...
import NavTrains from "./NavTrains";
...
const TrainRoute = () => {
  ...
  useEffect(() => {
    ...
    const paymentActions = roles
      .filter((role) => role.module === "trainRoutes")
  ...
  useEffect(() => {
    ...
    axios
      .get("https://bcknd.ticket-hub.net/api/admin/trainRoutes", {
        headers: {
  ...
  const handleDelete = (index, userName) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(
            `https://bcknd.ticket-hub.net/api/admin/trainRoute/delete/${index}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
