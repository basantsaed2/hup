...
const User = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get("https://bcknd.ticket-hub.net/api/admin/users", {
        headers: {
  ...
  const handleDelete = (index, userName) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(
            `https://bcknd.ticket-hub.net/api/admin/user/delete/${index}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
        ...
        <table className="w-full border-y border-x border-black ">
          <thead className="w-full ">
            <tr className="bg-four w-[1012px] h-[56px]">
              <th className="w-[10px] h-[56px] text-[14px] border-b text-left px-1">
                S/N
              </th>
              <th className="w-[200px] h-[56px] text-[14px] border-b text-left ">
                User
              </th>
              <th className="w-[200px] h-[56px] text-[14px] border-b   lg:text-center xl:text-start">
                Gmail
              </th>
              ...
              {paginatedData.map((item, index) => (
                ...
                >
                  ...
                  <td className="flex flex-col w-[200px] h-[56px] absolute top-1 gap-1 items-start justify-start">
                    <span className=" lg:text-[10px] xl:text-[12px]   font-normal text-five px-1">
                      {item?.name ?? "N//A"}
                    </span>
                    <span className="  lg:text-[10px] xl:text-[12px]   font-normal text-five px-1">
                      {item?.phone ?? "N//A"}
                  ...
                  <td className="w-[200px]   h-[56px] text-[10px]  px-1 ">
                    {item?.email ?? "N//A"}
                  </td>
            ...
            <div className="w-[95%] bg-six">
              {paginatedData.map((item, index) => (
                <div key={index} className="flex flex-col gap-4 p-3">
                  <span>
                    S/N : {(currentPage - 1) * rowsPerPage + index + 1}{" "}
                  </span>
