...
const AdminRoles = () => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        setData(response.data.admin_position);
      })
      .catch(() => {
  ...
  const handleDelete = (index, userName) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(
            `https://bcknd.ticket-hub.net/api/admin/admin_role/delete/${index}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
  ...
  const handleEdit = (index) => {
    ...
    navigate("/AddAdminRoles", { state: { sendData } });
  };
  ...
  const show = (roles) => {
    if (roles.length > 0) {
      ...
      roles.forEach((role) => {
        if (!grouped[role.module]) {
          grouped[role.module] = [];
        }

        if (Array.isArray(role.action)) {
          grouped[role.module].push(...role.action);
        } else {
          grouped[role.module].push(role.action);
        }
      });
      ...
      Object.keys(grouped).forEach((mod) => {
        grouped[mod] = [...new Set(grouped[mod])];
      });
      ...
      <div style="overflow-x: auto;">
        ...
        ">
          <thead style="background-color: #f5f5f5;">
            <tr>
              <th style="border: 1px solid #ccc; padding: 10px; min-width: 120px;">Module</th>
    ...
    `;
      ...
      Object.entries(grouped).forEach(([module, actions]) => {
        message += `
      <tr>
  <td style="border:1px solid #ccc; padding:10px; max-width:150px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">
    ${module}
  </td>
            ...
            <tbody>
              {paginatedData.map((item, index) => (
                ...
                >
                  ...
                  <td className="w-[143px] h-[56px]  lg:text-[10px] xl:text-[12px]  underline  ">
                    <button
                      className="bg-three   text-white rounded-4xl px-4 py-2"
                      onClick={() => show(item.roles)}
                    >
                      View
                    </button>
                  </td>
