...
const Payments = () => {
  ...
  useEffect(() => {
    ...
    const paymentActions = roles
      .filter((role) => role.module === "payment")
  ...
  useEffect(() => {
    ...
    axios
      .get("https://bcknd.ticket-hub.net/api/admin/pending_payments", {
        headers: {
  ...
  const handelconfirm = (id, one, two) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .put(
            `https://bcknd.ticket-hub.net/api/admin/payment/confirm/${id}`,
            {},
            {
              headers: {
                Authorization: `Bearer ${token}`,
  ...
  const handelcancel = (id, one, two) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .put(
            `https://bcknd.ticket-hub.net/api/admin/payment/cancel/${id}`,
            {},
            {
              headers: {
                Authorization: `Bearer ${token}`,
            ...
            <tbody>
              {paginatedData.map((item, index) => (
                ...
                >
                  ...
                  <td className="w-[143px] h-[56px]  text-[14px] px-2 ">
                    {item?.amount ?? "N//A"}
                  </td>
                  <td className="w-[143px] h-[56px] text-[16px]">
                    <img
                      ...
                      src={
                        item.receipt_image === null
                          ? `data:image/png;base64,${item.receipt_image}`
                          : item.receipt_image
                      }
                      alt="Receipt"
                      onClick={() =>
                  ...
                  <td className="w-[143px]  h-[56px]  text-[14px]  text-nine  ">
                    <span className="bg-eight font-normal rounded-[8px]">
                      {item.status}
                    </span>
                  </td>
                  {item.status !== "canceled" ? (
                    <td className="w-[200px]     flex gap-1  items-center h-12  ">
                      {actions.includes("status") && (
                        <>
                          <button
                            className="bg-three px-1 py-2 rounded-[8px] text-white text-[12px] "
                            onClick={() =>
                              handelconfirm(
                                item.id,
                                item.amount,
                          ...
                          <button
                            className="bg-three px-1 py-2 rounded-[8px] text-white text-[12px]"
                            onClick={() =>
                              handelcancel(item.id, item.amount, item.travelers)
                            }
                          >
                            cancel
                          </button>
