...
const ReedemPoint = () => {
  ...
  useEffect(() => {
    ...
    const paymentActions = roles
      .filter((role) => role.module === "redeem_point")
  ...
  useEffect(() => {
    ...
    axios
      .get("https://bcknd.ticket-hub.net/api/admin/point", {
        headers: {
  ...
  const handleDelete = (index, busNumber) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(
            `https://bcknd.ticket-hub.net/api/admin/point/delete/${index}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
