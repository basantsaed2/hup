...
const Admins = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get("https://bcknd.ticket-hub.net/api/admin/admin", {
        headers: {
  ...
  const handleDelete = (index, name) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(
            `https://bcknd.ticket-hub.net/api/admin/admin/delete/${index}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
          ...
          .catch(() => {
            Swal.fire(
              "Error!",
              `There was an error while deleting ${name}.`,
              "error"
            );
          });
      } else {
        Swal.fire("Cancelled", `${name} was not deleted.`, "info");
      }
    });
  };
  ...
  const handleEdit = (index) => {
    ...
    navigate("/AddAdmins", { state: { sendData } });
  };
  ...
  const cheose = ["Filter", "name", "email", "phone", "position"];
  const labelMap = {
    Filter: "Filter",
    name: " name",
    email: "email",
          ...
          {paginatedData.map((item, index) => (
            <div key={index} className="flex flex-col gap-4 p-3">
              ...
              <div className="flex gap-4">
                ...
                <span>{item?.position?.name ?? "N//A"}</span>
              </div>
