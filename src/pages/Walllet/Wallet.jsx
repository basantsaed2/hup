...
const Wallet = () => {
    ...
    axios.get("https://bcknd.ticket-hub.net/api/admin/wallet_request", {
      headers: {
      ...
      const closeModal = () => {
        ...
        const handleStatusChange=(id,trag)=>{
            if(trag==="Pending")return
              axios.put(`https://bcknd.ticket-hub.net/api/admin/wallet_request/status/${id}`, {
                status:trag
              }, {
                headers: {
                    Authorization: `Bearer ${token}`,
  ...
  return (
              ...
              <table className="w-full  border-y border-x border-black">
                ...
                <tbody>
                  {paginatedData.map((item, index) => (
                      ...
                      <td className="w-[143px] h-[56px] text-[16px] ">{item?.user?.name??"N//A"}</td>
    ...
    <td className="w-[143px] h-[56px] text-[16px]">
            ...
            />
                      ...
                      <td className="w-[143px] h-[56px] text-[12px] ">{item?.user?.phone??"N//A"}</td>
                      <td className="w-[143px] h-[56px] text-[12px] ">{item?.user?.email??"N//A"}</td>
                      <td className="w-[143px] h-[56px] text-[14px] ">{item?.amount??"N//A"}</td>
